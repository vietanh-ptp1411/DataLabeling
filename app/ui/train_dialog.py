...
class TrainDialog(QDialog):
    def __init__(self, main_window):
        ...
        self.model_combo.lineEdit().setPlaceholderText(
            tr("Để trống = mặc định (chất lượng cao nhất)"))
        self.model_combo.setToolTip(
            tr("Để trống để dùng model nền mặc định, hoặc chọn file .pt\n"
        ...
        self.epochs_spin.setToolTip(
            tr("Số epoch tối đa — kết hợp Patience nên cứ để cao"))
        ...
        self.batch_spin.setToolTip(tr("Giảm xuống 2 nếu máy yếu / hết RAM"))
        ...
        self.patience_spin.setToolTip(
            tr("Early Stopping: không cải thiện sau N epoch thì tự dừng.\n"
        ...
        self.optimizer_combo.setToolTip(
            tr("auto: ultralytics tự chọn optimizer + learning rate\n"
        ...
        cells = [("Epochs", self.epochs_spin, 0, 0),
                 ...
                 (tr("Patience (early stop)"), self.patience_spin, 2, 0),
        ...
        self.pretrained_check.setToolTip(tr(
        ...
        log_head = QLabel(tr("LOG TRAIN"))
        log_head.setObjectName("sectionLabel")
        layout.addWidget(log_head)
        self.log = QPlainTextEdit()
        self.log.setReadOnly(True)
        self.log.setMaximumBlockCount(5000)
        self.log.setPlaceholderText(tr("Log train sẽ hiện ở đây…"))
        layout.addWidget(self.log, stretch=1)

    def _browse_model(self):
        from app.training.trainer import app_models_dir
        p, _ = QFileDialog.getOpenFileName(
            self, tr("Chọn model .pt để train tiếp"), app_models_dir(),
            "PyTorch model (*.pt)")
        if p:
            self.model_combo.setCurrentText(p)

    def _browse_yaml(self):
        p, _ = QFileDialog.getOpenFileName(self, tr("Chọn data.yaml"), "",
                                           "YAML (*.yaml *.yml)")
        if p:
    ...
    def _start(self):
        ...
        if not data:
            QMessageBox.warning(self, tr("Thiếu"),
                                tr("Chọn file data.yaml (Export trước)."))
            return
    ...
    def _done(self, model_paths):
        ...
        self.log.appendPlainText(
            tr("\n=== XONG. Model đã lưu ===\n{p}").format(p=model_paths))
        QMessageBox.information(self, tr("Train xong"),
                                tr("Model đã lưu:\n{p}").format(p=model_paths))
    ...
    def closeEvent(self, event):
        if self.worker and self.worker.isRunning():
            QMessageBox.warning(
                self, tr("Đang train"),
                tr("Bấm Dừng và đợi train kết thúc trước khi đóng."))
            event.ignore()
            return
